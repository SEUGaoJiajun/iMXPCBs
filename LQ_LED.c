...
【最后更新】2019年03月12日 
【返回值】无
【参数值】灯的颜色  LED_t color     
【例子  】LED_Color(red);   //红灯亮
QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ*/
...
void Test_LED(void)
{
  ...
  {          
        ...
        for(int i = 0; i < 20; i++)
        {
            LED_Color_Reverse(green);   //绿灯闪烁
            delayms(100);
        }
        for(int i = 0; i < 20; i++)
        {
            LED_Color_Reverse(blue);    //蓝灯闪烁
            delayms(100);
        }
        for(int i = 0; i < 20; i++)
        {
            LED_Color_Reverse(violet);  //紫灯闪烁
            delayms(100);
        }
        for(int i = 0; i < 20; i++)
        {
            LED_Color_Reverse(yellow);  //黄灯闪烁
            delayms(100);
        }
        for(int i = 0; i < 20; i++)
        {
            LED_Color_Reverse(cyan);    //青灯闪烁
            delayms(100);
        }
        for(int i = 0; i < 20; i++)
        {
            LED_Color_Reverse(white);   //白灯闪烁
            delayms(100);
        }
  }
}
