...
{ 
  ...
  //TFTSPI_P16x16Str(0,0,"东南大学创意下棋",u16WHITE,u16BLACK);		//字符串显示
  TFTSPI_P8X16Str(0,0," seu smart chess",u16ORANGE,u16BLACK);		//字符串显示  
  ...
  /*
  float Angle = 45;             //这些变量我放在了LQ_UART
  float Vx = 75;
  float Vy = 75;
  float Vz = 1;
  bool UpGetChessFlag = 1;     //   UpFlag=1且DownFlag=1   前者靠上位机摄像头，后者靠下位机光电管，上位机发送UpFlag
  bool UpSetChessFlag = 0;
  bool UpSetBarrFlag = 0;
  */
  
  float res = 0;                //   预留
  float count = 1;              //   用于测试刷新率是否合适
  
  while(1)
  {
      sprintf(txt1, "Angle: %5.1f", Angle);                    //将变量填充到字符串的对应位置，并将字符串存放到txt[]中
      sprintf(txt2, "Vx: %5.1f", Vx);                    //将变量填充到字符串的对应位置，并将字符串存放到txt[]中
      sprintf(txt3, "Vy: %5.1f", Vy);                    //将变量填充到字符串的对应位置，并将字符串存放到txt[]中
      sprintf(txt4, "Vz: %5.1f", Vz);                    //将变量填充到字符串的对应位置，并将字符串存放到txt[]中
      sprintf(txt5, "GetChess: %5.1f", (float)UpGetChessFlag);                    //将变量填充到字符串的对应位置，并将字符串存放到txt[]中
      sprintf(txt6, "SetChess: %5.1f", (float)UpSetChessFlag);                    //将变量填充到字符串的对应位置，并将字符串存放到txt[]中
      sprintf(txt7, "SetBarr: %5.1f",  (float)UpSetBarrFlag);                    //将变量填充到字符串的对应位置，并将字符串存放到txt[]中
      sprintf(txt8, "other: %5.1f", res);                    //将变量填充到字符串的对应位置，并将字符串存放到txt[]中
      sprintf(txt9, "variate: %5.1f", count);                    //将变量填充到字符串的对应位置，并将字符串存放到txt[]中
      
      TFTSPI_P8X16Str(0, 1, txt1, u16WHITE,u16BLACK);              //将txt中 内容显示出来
      TFTSPI_P8X16Str(0, 2, txt2, u16WHITE,u16BLACK);              //将txt中 内容显示出来
      TFTSPI_P8X16Str(0, 3, txt3, u16WHITE,u16BLACK);              //将txt中 内容显示出来
      TFTSPI_P8X16Str(0, 4, txt4, u16WHITE,u16BLACK);              //将txt中 内容显示出来
      TFTSPI_P8X16Str(0, 5, txt5, u16WHITE,u16BLACK);              //将txt中 内容显示出来
      TFTSPI_P8X16Str(0, 6, txt6, u16WHITE,u16BLACK);              //将txt中 内容显示出来
      TFTSPI_P8X16Str(0, 7, txt7, u16WHITE,u16BLACK);              //将txt中 内容显示出来
      TFTSPI_P8X16Str(0, 8, txt8, u16WHITE,u16BLACK);              //将txt中 内容显示出来
      TFTSPI_P8X16Str(0, 9, txt9, u16WHITE,u16BLACK);              //将txt中 内容显示出来
      delayms(300);
      count *= 1.1; 
      if(count > 200){
        count = 1;
