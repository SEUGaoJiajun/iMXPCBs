...
{  
    ...
    IOMUXC_SetPinMux(IOMUXC_GPIO_EMC_41_GPIO3_IO27, 0U);  //新增管脚C7，不要初始LED了
    ...
    GPIO_PinInit(GPIO3,  27, &GPIO_Input_Config);        // GPIO输入口，非中断
...
uint8_t LQ_KEY_Read(uint8_t mode)
{
    ...
    if(key_up && (GPIO_PinRead(GPIO2, 27)==0 || GPIO_PinRead(GPIO2, 30)==0) || GPIO_PinRead(GPIO3, 4)==0  || GPIO_PinRead(GPIO3,  27)==0)
    {
      delayms(10);   //消抖
      key_up=0;
      ...
      else if(GPIO_PinRead(GPIO3,  27)==0) 
      {
          return 4;    
      }
      
    }
    if(GPIO_PinRead(GPIO2, 27)==1 && GPIO_PinRead(GPIO2, 30)==1 && GPIO_PinRead(GPIO3,  4)==1 && GPIO_PinRead(GPIO3,  27)==1 ) 
    {
     key_up=1;   
    }
    return 0;   //无按键按下
...
/*LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
【作  者】CHIUSIR
【功能说明】GPIO口中断测试   按下核心板WUP按键，显示1s红灯
【软件版本】V1.0
【最后更新】2018年11月24日 
【函数名】
【返回值】无
